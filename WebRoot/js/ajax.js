...
function getVerifyCode(){
	...
	xhr.open("POST","./servlet/VerifyCode");
	xhr.setRequestHeader("CONTENT-TYPE","application/x-www-form-urlencoded");
	xhr.send(null);
	
	xhr.onreadystatechange = function(){
		if (xhr.readyState == 4 && xhr.status == 200) {
			codeImage.src = "./servlet/VerifyCode?time="+
				new Date().getTime();
		}
	}
}
...
function login(){
	...
	xhr.onreadystatechange = function(){
		if (xhr.readyState == 4 && xhr.status == 200) {
			...
			if (rt == "login000") {	//登陆成功
				alert("登陆成功");
				location.href = "/Enterprise/console/home.jsp";
				return true;
...
function newsRelease(){
	...
	isShow = encodeURIComponent(isShow);
	...
	xhr.open("POST","servlet/NewsRelease");
	xhr.setRequestHeader("CONTENT-TYPE","application/x-www-form-urlencoded");
	xhr.send(param);
...
function getNewsList(){
	...
	$.ajax({
		url:"servlet/NewsList",
		type:"POST",
		
		success:function(e){
			console.log("getNewsList success!");
		}
	});
}
